...
public class SimphonyTest{
	...
	private static void registerPlatforms() {
		...
		DialogFixture wd = WindowFinder.findDialog(CompNames.Dial.WIZARD).using(robot);
		...
		jfc.setCurrentDirectory(new File(projectPath+sep+"Platforms"+sep+"Simphony2.0"));
		jfc.approve();
	...
	@GUITest
    @Test
    public void testSimphony()
    {
    	Pause.pause(1500);
       	wizardDialog = WindowFinder.findDialog(CompNames.Dial.WIZARD).using(robot);
       	
       	int methodIndex = setModel("simphony", "Model4Meme.class","manual", true);

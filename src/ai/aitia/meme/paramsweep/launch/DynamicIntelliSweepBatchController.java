...
public class DynamicIntelliSweepBatchController implements IBatchController, IBatchListener {
	...
	//----------------------------------------------------------------------------------------------------
	public DynamicIntelliSweepBatchController(IIntelliDynamicMethodPlugin plugin) {
		this.plugin = plugin;
		actBatchController = PlatformManager.getPlatform(PlatformSettings.getPlatformType()).getBatchController();
	...
	public void batchEnded(BatchEvent event) {
		...
		if (plugin.noMoreIteration() || interrupted) {
			renameRecorderFiles();
			platform.prepareResult(recorders,new File("."));
			deleteFileParts();

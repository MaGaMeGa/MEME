...
public class Operator_Remove implements IListOperatorPlugin {
	...
	public String getLocalizedName() { return "Remove element"; }
	...
	public String getDescription() {
		...
		if (PlatformSettings.getPlatformType() == PlatformType.NETLOGO || PlatformSettings.getPlatformType() == PlatformType.NETLOGO5)
			...
			desc += "array"; 
		return desc + "except the first instance of the given element.";
	}
	...
	private String getJavaCode(MemberInfo info, Class<?> innerType, MemberInfo object) {
		...
		code.append("result.remove(" + element(object,boxingType(innerType)) + ");\n");
	...
	private String getNetLogoCode(MemberInfo info, MemberInfo object) {
		...
		if (info.getJavaType().equals(list.class)) {
			code.append("let local0 position ").append(object.getName()).append(" ").append(info.getName()).append("\n");
			code.append("ifelse local0 != false\n");
			code.append("[report remove-item local0 ").append(info.getName()).append("]\n");

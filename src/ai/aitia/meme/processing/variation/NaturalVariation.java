...
public class NaturalVariation implements ActionListener, IIntelliGenericProcesser{
	...
	private ResultInMem first = null;
	private boolean needRandomSeedStatistics = false;
	private boolean needNaturalVariation = true;
//	protected Vector<Integer> seedValuesSize = null;
//	protected Vector<Double> uniqueSampleSeedVariances = null;
//	protected Vector<Object[]> uniqueSamplesSeed = null;
//	protected String[] rngSeedNamesForSeedStats = null;
//	protected String[] observedSeed = null;
//	protected int[] observedIndicesSeed = null;
//	protected Vector<String> allInParameterNames = null;
	...
	public boolean loadAndProcessData(List<ResultInMem> results, Element pluginElement) {
		...
		if(rsmElem != null){
			...
			if (needNaturalVariation) {
				...
				for (int i = 0; i < uniqueSamples.size(); i++) {
					...
					List<ResultInMem> filtered = filterResults(results, observed, uniqueSamples.get(i));
					...
					for (int j = 0; j < filtered.size(); j++) {
						ResultInMem run = filtered.get(j);
					...
					for (int j = 0; j < fullUniqueSamples.size(); j++) {
						final List<ResultInMem> filteredFull = filterResults(filtered, parameters, fullUniqueSamples.get(j));
					...
					Double[] weighedStdDevs = weighedArithmeticMean(stdDevs, populations);
					...
					Double[] weighedCoeffOfVariations = weighedHarmonicMean(coeffOfVariations, populations);
			...
			if (needRandomSeedStatistics) {
				...
				for (int rngSeedIndex = 0; rngSeedIndex < rngSeedNames.length; rngSeedIndex++) {
					...
					for (int i = 0; i < uniqueSamplesSeed.size(); i++) {
						...
						List<ResultInMem> filtered = filterResults(results, observedSeed, uniqueSamplesSeed.get(i));
						Vector<Object[]> fullUniqueSamples = new Vector<Object[]>();
						...
						final Vector<Double[]> stdDevs = new Vector<Double[]>();
						for (int j = 0; j < fullUniqueSamples.size(); j++) {
							final List<ResultInMem> filteredFull = filterResults(filtered, parametersForSeeds, fullUniqueSamples.get(j));
						...
						uniqueSampleSeedCoeffOfVars.add(weighedHarmonicMean(coeffOfVariations, populations));
						uniqueSamplePopulations.add(population);
					}
					seedStdDevs.add(weighedArithmeticMean(uniqueSampleSeedStdDevs, uniqueSamplePopulations));
					seedCoefficientOfVariations.add(weighedHarmonicMean(uniqueSampleSeedCoeffOfVars, uniqueSamplePopulations));
				}
			}
		}
	...
	public static Double[] weighedHarmonicMean(final List<Double[]> elements, final List<Double> weights) {
		if (elements == null || weights == null || elements.size() == 0 || weights.size() == 0 || elements.size() != weights.size()) {
			throw new IllegalArgumentException("Arguments of weighed harmonic mean cannot be null, have 0 elements or have differents sizes.");
		}
		final Double[] ret = new Double[elements.get(0).length];
		double sumW = 0.0;
		for (Double weight : weights) {
			sumW += weight;
		}
		double[] denominator = new double[ret.length];
		for (int i = 0; i < denominator.length; i++) {
			denominator[i] = 0.0;
		}
		for (int i = 0; i < elements.size(); i++) {
			for (int j = 0; j < denominator.length; j++) {
				denominator[j] += (weights.get(i) / elements.get(i)[j]);
			}
		}
		for (int i = 0; i < denominator.length; i++) {
			ret[i] = sumW / denominator[i];
		}
		return ret;
	}

	public static Double[] weighedArithmeticMean(final List<Double[]> elements, final List<Double> weights) {
		if (elements == null || weights == null || elements.size() == 0 || weights.size() == 0 || elements.size() != weights.size()) {
			throw new IllegalArgumentException("Arguments of weighed arithmetic mean cannot be null, have 0 elements or have differents sizes.");
		}
		final Double[] ret = new Double[elements.get(0).length];
		double sumW = 0.0;
		for (Double weight : weights) {
		...
		double[] numerator = new double[ret.length];
		for (int i = 0; i < numerator.length; i++) {
			numerator[i] = 0.0;
		}
		for (int i = 0; i < elements.size(); i++) {
			for (int j = 0; j < numerator.length; j++) {
				numerator[j] += (weights.get(i) * elements.get(i)[j]);
			}
		}
		for (int i = 0; i < numerator.length; i++) {
			ret[i] = numerator[i] / sumW;
		}
		return ret;
	}

	public static Row getLastRow(ResultInMem result){
	...
	public static List<ResultInMem> filterResults(List<ResultInMem> runs, String filterColumnName, Object filterValue) {
		...
		if (filterColumnIdx != -1){
			for (final ResultInMem run : runs) {
	...
	public List<ResultInMem> getVariationResults(IResultsDbMinimal db) {
		//the result:
	...
	public JPanel getGenericProcessPanel(final Element pluginElement){
		if (this.pluginElement == null || this.genericProcesserPanel == null) {
			...
			NodeList nl = null;
			nl = this.pluginElement.getElementsByTagName(RngSeedManipulatorModel.NATURAL_VARIATION_ELEMENT);
			if(nl!=null && nl.getLength() > 0){
			...
			final JPanel checkboxesPanelPanel = FormsUtils.build(
					"~ p ~", 
					"_ p||" +
					"0 p||", 
					checkboxesPanel

...
public class Operator_MaxPlaces implements IListOperatorPlugin {
		
	//====================================================================================================
	// implemented interfaces
	
	//----------------------------------------------------------------------------------------------------
	public String getName() { return "maxPlaces"; }
	public String getLocalizedName() { return "Maximum places"; }
	...
	public String getDescription() {
		String desc = "Returns the list of the indices of the maximum values in the specified list";
		...
		if (!(actParams[1] instanceof Class)) 
			...
			if (PlatformSettings.getPlatformType() != PlatformType.NETLOGO && PlatformSettings.getPlatformType() != PlatformType.NETLOGO5) {
				if (innerType.isPrimitive() && !innerType.equals(Boolean.TYPE))
					return result;
				if (!Comparable.class.isAssignableFrom(innerType) || innerType.equals(Boolean.TYPE))
	...
	private String getJavaCode(MemberInfo mi, Class<?> innerType) {
		...
		if (mi.getType().endsWith("[]")) {
			...
			code.append("}\n");
			code.append(innerType.getName() + " max = " + mi.getName() + "[0];\n");
			code.append("result.add(new Integer(0));\n");
			code.append("for (int i = 1; i < " + mi.getName() + ".length;++i) {\n");
			if (innerType.isPrimitive()) {
				code.append("if (" + mi.getName() + "[i] > max) {\n");
					code.append("result.clear();\n");
					code.append("max = " + mi.getName() + "[i];\n");
					code.append("result.add(new Integer(i));\n");
				code.append("} else if (" + mi.getName() + "[i] == max) {\n");
					code.append("result.add(new Integer(i));\n");
				code.append("}\n");
			} else {
				code.append("if (((java.lang.Comparable)" + mi.getName() + "[i]).compareTo(max) > 0) {\n");
					code.append("result.clear();\n");
					code.append("max = " + mi.getName() + "[i];\n");
					code.append("result.add(new Integer(i));\n");
				code.append("} else if (((java.lang.Comparable)" + mi.getName() + "[i]).compareTo(max) == 0) {\n");
					code.append("result.add(new Integer(i));\n");
				code.append("}\n");
			}
			code.append("}\n");
			...
			code.append("if (" + mi.getName() + ".size() == 0) {\n");
				code.append("return result;\n");
			code.append("}\n");
			code.append(innerType.getName() + " max = " + mi.getName() + ".get(0);\n");
			code.append("result.add(new Integer(0));\n");
			code.append("for (int i = 1; i < " + mi.getName() + ".size();++i) {\n");
				code.append("if (((java.lang.Comparable)" + mi.getName() + ".get(i)).compareTo(max) > 0) {\n");
					code.append("result.clear();\n");
					code.append("max = " + mi.getName() + ".get(i);\n");
					code.append("result.add(new Integer(i));\n");
				code.append("} else if (((java.lang.Comparable)" + mi.getName() + ".get(i)).compareTo(max) == 0) {\n");
					code.append("result.add(new Integer(i));\n");
				code.append("}\n");
			code.append("}\n");
		}
	...
	private String getNetLogoCode(MemberInfo mi) {
		StringBuilder code = new StringBuilder();
		code.append("let _max max ").append(mi.getName()).append("\n");
		code.append("let result []\n");
		code.append("foreach n-values length ").append(mi.getName()).append(" [?] [\n");
		code.append("if item ? ").append(mi.getName()).append(" = _max [\n");

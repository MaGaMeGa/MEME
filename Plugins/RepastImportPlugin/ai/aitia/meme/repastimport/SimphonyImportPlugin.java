...
public class SimphonyImportPlugin implements IImportPlugin {

	//-------------------------------------------------------------------------
	public String getLocalizedName() {
		return "Repast Simphony result file";
	}

	//-------------------------------------------------------------------------
	public java.awt.Window showDialog(IImportPluginContext ctx) {
		String resultType = "Repast Simphony";
		java.io.File f[] = null;
	    ...
	    if (f != null && f.length != 0) {
	    	if (f.length == 1) {
	    		RepastImportDialog d = new RepastImportDialog(ctx,resultType,true);
	    		d.start(f[0]);
	    	} else {
	    		RepastMultiImportDialog d = new RepastMultiImportDialog(ctx,resultType,true);
	    		d.start(f);
	    	}
	    }

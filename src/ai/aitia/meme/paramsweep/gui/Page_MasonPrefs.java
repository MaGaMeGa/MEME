...
public class Page_MasonPrefs extends PreferencesPage implements IReinitalizeable, ActionListener {
	//=====================================================================================
	//members
	 ...
	 */
	public Page_MasonPrefs(WizardPreferences owner) {
		super("MASON");
		this.owner = owner;
		layoutGUI();
		reinitialize();
	...
	//-------------------------------------------------------------------------------------
	@Override public String getInfoText(Preferences p) { return "MASON platform properties."; }
	
	//-------------------------------------------------------------------------------------
	public void reinitialize() {
		Properties p = owner.getProperties();
		sourceCheckBox.setSelected(Boolean.parseBoolean(p.getProperty(WizardPreferences.MASON_SOURCE_GENERATION)));
		closeAfterOneScriptCheckBox.setSelected(Boolean.parseBoolean(p.getProperty(WizardPreferences.MASON_CLOSE_AFTER_ONE_SCRIPT)));
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if ("REGISTER".equals(cmd)) {
			PlatformManager.registerPlatform(PlatformType.MASON,".");
			...
			if (result == 1) {
				PlatformManager.removePlatform(PlatformType.MASON);
	...
	public boolean onButtonPress(Button b) {
		...
		case OK		: Properties p = owner.getProperties();
					  p.setProperty(WizardPreferences.MASON_SOURCE_GENERATION,String.valueOf(sourceCheckBox.isSelected()));
					  p.setProperty(WizardPreferences.MASON_CLOSE_AFTER_ONE_SCRIPT,String.valueOf(closeAfterOneScriptCheckBox.isSelected()));
		}
		return true;
	}

...
public class NetLogo_503_Test{
    ...
    @GUITest
    @Test
    public void testNlogo()
    {
    	Pause.pause(1000);
       	wizardDialog = WindowFinder.findDialog(CompNames.Dial.WIZARD).using(robot);
       	
       	
       	int methodIndex = setModel("netlogo 5.0.3", "Fire5.nlogo","static", true);
       	if(methodIndex==0)setParameters(false);
        setRecorders(false);
		...
		DialogFixture nlogoDialog = WindowFinder.findDialog(CompNames.Dial.NLOGOIMPORT).withTimeout(30,TimeUnit.MINUTES).using(robot);
		Pause.pause(1000);
        nlogoDialog.textBox(CompNames.Fld.NLogoImport.MODELNAME).deleteText();
        nlogoDialog.textBox(CompNames.Fld.NLogoImport.MODELNAME).enterText("nlogo_503 model test");
        
        nlogoDialog.textBox(CompNames.Fld.NLogoImport.BATCHNAME).deleteText();
        nlogoDialog.textBox(CompNames.Fld.NLogoImport.BATCHNAME).enterText("test123");
        

        nlogoDialog.button(CompNames.Btn.OK).click();
    ...
    private void setRecorders(boolean skip)
    {
    	if(skip)
    	{
    		...
    		wizardDialog.list(CompNames.Lst.Recorders.VARIABLES).
    			selectItem(Pattern.compile(".*int.*|.*string.*|.*boolean.*|.*long.*|.*double.*", Pattern.CASE_INSENSITIVE));
            wizardDialog.button(CompNames.Btn.Recorders.ADD).click();
    		return;
    	}
    	wizardDialog.textBox(CompNames.Fld.Recorders.STOP).setText("20");
    	wizardDialog.button(CompNames.Btn.Recorders.ADDRECORDER).click();
        wizardDialog.list(CompNames.Lst.Recorders.VARIABLES).selectItem(Pattern.compile(".*burned.*", Pattern.CASE_INSENSITIVE));
        wizardDialog.button(CompNames.Btn.Recorders.ADD).click();
        wizardDialog.list(CompNames.Lst.Recorders.VARIABLES).selectItem(Pattern.compile(".*initial.*", Pattern.CASE_INSENSITIVE));
        wizardDialog.button(CompNames.Btn.Recorders.ADD).click();
    }
...
//        wizardDialog.textBox(CompNames.Fld.Parameters.STEP).deleteText();
        wizardDialog.textBox(CompNames.Fld.Parameters.START).enterText("40");
        wizardDialog.textBox(CompNames.Fld.Parameters.END).enterText("60");
